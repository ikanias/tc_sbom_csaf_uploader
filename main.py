...
class TcUpload(cmd.Cmd):
    global token
    def install_oidc(self, idp_url, client_name, client_id, client_secret, certificate_path):
        ...
        print(output1)
        command = 'oidc create confidential ' + '--issuer ' + idp_url + ' --client-id ' + client_id + ' --client-secret ' + client_secret + " " + client_name + ' --root-certificate ' + cert_tls_path
        subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
if __name__ == '__main__':
    ...
    idp_url = input("Please enter your idp server issuer url(i.e. https://foo.bar/user_pool_id) from the idp console: ") # Enter the cognito URL with the user pool id
